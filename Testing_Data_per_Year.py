...
# depending on the data output choice by user (whole dataset, or each year)
...
def test_data():
   # Asking the user for choice
    ...
    if choice == 1:
        # Splitting the data into training and testing sets for the entire dataset
        ...
        for idx, (X_train, X_test, y_train, y_test) in enumerate(train_test_sets):
            ...
            print(f"Accuracy for Year {unique_years[idx]}: {accuracy:.2f}%  /  number of samples: {len(current_year_data)} samples")
            results_df = pd.DataFrame()
            results_df['Actual Fall Risk Level'] = y_test.values.ravel()
            results_df['Predicted Fall Risk Level'] = predictions
